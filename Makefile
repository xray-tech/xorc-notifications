SHELL = /bin/sh
executable = target/release/web-push
artifactory = http://repo:katti@artifactory.service.consul:8081/artifactory/web_push
...
upload: ## Upload the binary to the repository
	$(curl) -T $(executable) $(artifactory)/$(stage)/web-push-$(commit_id)
